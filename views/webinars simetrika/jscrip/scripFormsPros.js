...
$(document).on("change", "#CboPais", function () {
  ...
  $.ajax({
    data: {idPais},
    url: "https://simetrika.ga/controller/DevolverDatos.php",
    type: "post",
    beforeSend: function () {},
...
$("#btn_guardar").click(function (e) {
    ...
    if ($(`input:checkbox[id=chkTerminosCondiciones]:checked`).val()) {
        if (campos.txtEmail == true && campos.txtApePaterno == true && campos.txtApeMaterno == true && campos.txtnombres == true && campos.txtNumCelular == true && campos.CboPais == true /* && campos.respuesta6==true && campos.respuesta7 */
        ) {
...
function TraerCursos() {
    ...
    $.ajax({
        ...
        <div class="contenChecks">
          <input type="radio" name="rbOcupacion" class="cheks" name="" id="${
                    element.id
                }" value="${
                    element.id
                }">
...
const validarFormulario = (e) => {
    switch (e.target.name) {
        case "txtEmail": validarCampo(expresiones.correo, e.target, "txtEmail", "Ingrese fromato corecto Ej: micorreo12@gmail.com");
            break;
        case "txtApePaterno": validarCampo(expresiones.nombre, e.target, "txtApePaterno", "Ingrese correctamente apellido paterno");
            break;
        case "txtApeMaterno": validarCampo(expresiones.nombre, e.target, "txtApeMaterno", "Ingrese correctamente apellido materno");
            break;
...
function fnValidarDatosExistentes(estado, campo, msj) {
    if (estado == true) {
        ...
        document.querySelector(`#grupo__${campo} .formulario__grupo-input i`).classList.add("fa-check-circle");
        document.querySelector(`#grupo__${campo} .mensaje`).classList.remove("mensaje-error");
        document.querySelector(`#grupo__${campo} .mensaje`).classList.add("mensaje-exito");
        document.querySelector(`#grupo__${campo} .mensaje`).innerHTML = msj;
